...
const messages = {
    ...
    lost: "Otrzymałeś potężny cios, który niemalże zwalił cię z nóg!",
...
const fightingBar = () => {
    ...
    const move = () => {
        ...
        pos = Math.max(
            pos + (speed + winCounter * 0.3 + playerStats.level * 0.3 + monsterStats.power * 0.3) * direction,
            width - 100,
            Math.min(
                pos + (speed + winCounter * 0.3 + playerStats.level * 0.3 + monsterStats.power * 0.3) * direction,
                100 - width
...
const updateProgress = () => {
    ...
    if ((playerStats.power * 2 * winCounter) - monsterStats.toughness <= 0) {
        ...
        progress =
            progress +
            parseInt(
                (playerStats.power * 2 * winCounter) - monsterStats.toughness
            );
    }
...
const updateHP = () => {
    // console.log(hp);
    if ((2 * monsterStats.power - (1.5 * playerStats.toughness) +
            monsterLevelStats) < 3) {
        hpActual = hpActual - 2;
    }
    hpActual =
        hpActual -
        (2 * monsterStats.power - (1.5 * playerStats.toughness) +
            monsterLevelStats);
...
const updateLives = () => {
    if (hpActual <= 0 && lostCounter === 0) {
        ...
        newLevel.style.display = "unset";
        newLevel.innerHTML =
            "Niestety! Walka z " + monsterStats.name + " zakończyła się twoją porażką";
        hpActual = 100 * (1 + (playerStats.level + playerStats.toughness) / 10);
        lostCounter++;
        return;
    }
